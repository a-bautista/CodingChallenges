'''
    ...
    We have to find the node with the nthnthnth highest value.
...
class Solution:
    ...
    def findNHighestNode(self, root, n):
        ...
        # traverse recursively the right root
        rightBranch = self.findNHighestNode(root.right, n)

        # if you have all the right nodes and you ended up to the right most node, then
        # you have the highest node value
        if rightBranch is not None:
            return rightBranch

        # you count when you traverse the left branch because you want the n highest node
        self.count +=1
        if n == self.count:
            return root.val

        # traverse the left branch to find the nth node
